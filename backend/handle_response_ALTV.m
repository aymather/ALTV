...
function [RT,Resp] = handle_response_ALTV(daq,ms,settings,trialseq,id,it,stim,side)
...
while GetSecs - starttime < ms/1000
    ...
    % STIMULUS DISPLAY
    if GetSecs - starttime >= settings.duration.delay && ~vdisplay
        ...
        DrawFormattedText(settings.screen.outwindow, stim, settings.screen.outwindowdims(3)/2+side, 'center', settings.layout.color.text);
